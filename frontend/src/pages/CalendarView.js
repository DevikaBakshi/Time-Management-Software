  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <Calendar
        ...
        value={selectedDate}
      />
      <h3 style={{ marginTop: "20px" }}>Meetings on {selectedDate.toDateString()}</h3>
        ...
        <ul>
          {meetingsForDate.map((meeting) => (
            <li key={meeting.meeting_id}>
              <strong>{meeting.title}</strong> from{" "}
              {new Date(meeting.start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} to{" "}
